...
# ── Sidebar ────────────────────────────────────────────────────────────────────
with st.sidebar:
    st.markdown("**Visualization Engine** · v2.0")
    st.markdown("---")

    # ── Mode: Pre-built vs AI-generated ─────────────────────────────────────
    mode = st.radio(
        "Mode",
        ["📦  Pre-built Concepts", "🤖  AI-Generated (Any Topic)"],
        index=0,
    ...
    if not is_ai_mode:
        ...
        custom_topic = st.text_input(
            "💬 Topic",
            placeholder="e.g. merge sort, gradient descent, CNN...",
            help="Type ANY CS/AI concept — the LLM will generate a visualization",
        )
        custom_operation = st.text_input(
            "🔧 Focus / Operation (optional)",
            placeholder="e.g. step-by-step, forward pass...",
        )
        ...
        cfg = {
            "topic": custom_topic.strip() if custom_topic else "unknown",
            "operation": custom_operation.strip() if custom_operation else "explain",
            "level": custom_level,
        }
        ...
        if groq_key:
            st.markdown("""
            <div style='background:#14532d; border-radius:8px; padding:0.5rem 0.8rem;
                        font-size:0.78rem; color:#86efac; margin:0.5rem 0;'>
                ✅ Groq API connected · Llama 3.3 70B
            </div>""", unsafe_allow_html=True)
        else:
            st.markdown("""
            <div style='background:#7f1d1d; border-radius:8px; padding:0.5rem 0.8rem;
                        font-size:0.78rem; color:#fca5a5; margin:0.5rem 0;'>
                ⚠️ Enter a Groq API key to enable AI generation
            </div>""", unsafe_allow_html=True)
    ...
    can_generate = True
    if is_ai_mode and (not cfg.get("topic") or cfg["topic"] == "unknown"):
        can_generate = False

    if st.button("▶  Generate & Animate", key="gen_btn", disabled=not can_generate):
        gen_label = f"🤖 AI generating: {cfg.get('topic', '')}" if is_ai_mode else "Generating visualization"
        with st.spinner(gen_label + "…"):
    ...
    <b>Pre-built</b><br>
    🔗 Linked List · 🔍 Binary Search · 🌀 Recursion<br>
    👁️ Transformer · 🧠 Neural Network · 📚 RAG Pipeline<br><br>
    <b>AI-Generated</b> (requires Groq API key)<br>
    Any topic: sorting, graphs, diffusion, GANs...
    </div>
    """, unsafe_allow_html=True)
