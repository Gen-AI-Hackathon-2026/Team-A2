...
# ---------------------------------------------------------------------------
# Global CSS + FOUC-free Theme System
# ---------------------------------------------------------------------------
def inject_global_styles():
    ...
    # Inject theme attribute synchronously to prevent FOUC
    fouc_js = f"""<script>
(function(){{
...
# Theme Helpers
# ---------------------------------------------------------------------------
def _persist_theme_js(theme: str) -> str:
    """Inject JS that writes theme to localStorage and updates data-theme attr."""
    return (
        f'<script>(function(){{'
...
# ---------------------------------------------------------------------------
# Navigation (Sidebar & Top Context Bar)
# ---------------------------------------------------------------------------
def render_sidebar_nav():
    ...
    with col1:
        topic_label = st.session_state.selected_topic or "Home"
        st.markdown(
    ...
    with col2:
        st.text_input("", key="global_search", label_visibility="collapsed",
    ...
    with st.sidebar:
        ...
        if st.button(toggle_lbl, use_container_width=True, key="theme_toggle"):
            ...
            st.markdown(_persist_theme_js(new_theme), unsafe_allow_html=True)
            st.rerun()
        ...
        st.markdown(
            '<a href="http://localhost:8000" target="_self" style="text-decoration:none;">'
            '<button style="width:100%;background:transparent;color:var(--text-muted);'
            'border:1px solid var(--border);padding:7px 14px;border-radius:8px;'
            'cursor:pointer;font-family:Inter,sans-serif;font-weight:500;font-size:0.8rem;">'
            '← Exit to Hub'
            '</button></a>',
            unsafe_allow_html=True,
        )
