...
def inject_global_styles():
    ...
    /* Hide the collapse/expand arrow so sidebar can't be hidden */
    ...
    with st.sidebar:
        ...
        if st.session_state.selected_topics:
            ...
            for t in st.session_state.selected_topics[:4]:
                ...
                <div style="color:#00D2FF;font-size:0.78rem;padding:0.18rem 0.5rem;
                            background:rgba(0,210,255,0.06);border-radius:6px;
                            margin-bottom:0.18rem;">{short}</div>
                """, unsafe_allow_html=True)
            if len(st.session_state.selected_topics) > 4:
                st.markdown(
                    f"<div style='color:#6B6B8D;font-size:0.7rem;'>"
                    f"+{len(st.session_state.selected_topics)-4} more</div>",
                    unsafe_allow_html=True,
                )
            st.divider()
        ...
        if st.session_state.get("rag_initialized"):
            ...
            <div style="font-size:0.7rem;color:#A0A0C0;padding:0.35rem;">
                <div style="color:#2ECC71;font-weight:600;margin-bottom:0.2rem;">
                    RAG Active</div>
                <div>{status['num_chunks']} chunks | {status['num_vectors']} vectors</div>
                <div style="color:#8B83FF;margin-top:0.15rem;font-size:0.65rem;">
                    GraphRAG Enabled</div>
            </div>""", unsafe_allow_html=True)
        else:
            st.markdown("""
            <div style="font-size:0.7rem;color:#6B6B8D;padding:0.35rem;">
                RAG: Initializing...</div>
            """, unsafe_allow_html=True)
...
# ---------------------------------------------------------------------------
# Main — sidebar is the ONLY navigation, NO top nav bar
# ---------------------------------------------------------------------------
def main():
    configure_page()
