"""
voice_components.py — Shared Streamlit UI helpers for voice I/O
================================================================
Uses Streamlit's NATIVE st.audio_input() widget (available since 1.35.0).
No third-party audio-recorder package required — zero dependency conflicts.

Provides two reusable render functions used by both tutor.py and chatbot.py:

    render_voice_input(page_key)          → Optional[str]  (transcript)
    render_tts_controls(text, ...)        → None           (plays audio)
    render_tts_settings(page_key)         → bool           (auto-play toggle)
...
* TTS is triggered on explicit "🔊 Listen" button click or automatically when
  the auto-play toggle is enabled.
* All errors are surfaced as Streamlit warnings — never silent crashes.
"""
...
def render_voice_input(page_key: str) -> Optional[str]:
    ...
    if not transcript:
        st.warning("🎤 Could not understand the audio. Please try speaking again.")
        return None
