"""
student_memory.py — Student Digital Twin for Synapse AI Tutor
=============================================================
Single source of truth for:

  * Conversation History  — short-term, 20-message rolling window per topic
  * Quiz History          — structured quiz results with mistakes[]
  * Learning History      — educational events (quiz_completed, tutor_session)
  * Mistake Tracking      — wrong concepts surfaced per event, queryable
  * Recent Topics         — ordered list of topics studied (most-recent first)
  * Student Summary       — compact dict injected into tutor system prompt
...
def _get_student(data: dict, username: str) -> dict:
    ...
    Mutates ``data`` in place — caller must call save_student_memory()
    afterwards.
    """
...
# ---------------------------------------------------------------------------
# Part 1 — Conversation History (Short-Term Memory)
# ---------------------------------------------------------------------------
def add_message(username: str, topic: str, role: str, content: str) -> None:
    ...
    if role not in ("user", "assistant"):
        logger.warning(f"[StudentMemory] add_message(): invalid role '{role}' — skipped")
        return
...
def add_quiz_result(
    ...
    for questions the student answered incorrectly — used later by
...
def add_learning_event(
    ...
    **kwargs: Any,
    ...
    ---------------------------
    "quiz_completed"  — fired by pages/assessment.py
    "tutor_session"   — fired by pages/tutor.py on session start

    Common kwargs for "quiz_completed"
    -----------------------------------
        quiz_score    (int)  — percentage 0–100
        mastery_before (int) — mastery before this quiz
        mastery_after  (int) — mastery after this quiz (from progress_tracker)

    Common kwargs for "tutor_session"
    ----------------------------------
        questions_asked (int) — messages exchanged this session
        mastery_before  (int) — mastery at session start
    ...
    event: dict[str, Any] = {
        ...
        "topic":      topic,
        "mistakes":   mistakes or [],   # ← always present, even if []
    }
    event.update(kwargs)
...
# ---------------------------------------------------------------------------
# Part 4 — Derived Views (all mastery data delegated to progress_tracker)
# ---------------------------------------------------------------------------
def get_weak_topics(username: str) -> list[str]:
    """
    Return weak topic names (mastery < 50).

    Delegates entirely to progress_tracker — student_memory owns
...
# ---------------------------------------------------------------------------
# Part 5 — Student Summary (injected into tutor system prompt)
# ---------------------------------------------------------------------------
