"""
voice_config.py — Centralized voice layer configuration for Synapse AI Tutor
=============================================================================
...
"hackathon"  (DEFAULT)
    ...
    TTS  : gTTS only
    APIs : zero external dependencies — runs fully offline
...
"premium"
    STT  : Groq Whisper (cloud) → faster-whisper fallback
...
class VoiceConfig:
    ...
    # ── STT — faster-whisper (always available) ───────────────────────────────
    ...
    # ── STT — Groq (optional acceleration, never required) ────────────────────
    # The app MUST continue working when Groq is unavailable.
    # Groq is only an optimisation layer — never a dependency.
    ...
    def _apply_mode(cls) -> None:
        if cls.VOICE_MODE == "hackathon":
            ...
            logger.info("[VoiceConfig] Mode: HACKATHON — local-only, no cloud APIs")

        elif cls.VOICE_MODE == "standard":
            cls.USE_GROQ_STT = False
            # ElevenLabs allowed if key is present (env-controlled)
            logger.info("[VoiceConfig] Mode: STANDARD — local STT, gTTS primary")

        elif cls.VOICE_MODE == "premium":
            # Groq + ElevenLabs allowed — controlled by env vars
            logger.info("[VoiceConfig] Mode: PREMIUM — Groq STT, ElevenLabs optional")
