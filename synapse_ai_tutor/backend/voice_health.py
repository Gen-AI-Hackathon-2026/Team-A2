"""
voice_health.py — Voice layer startup health check for Synapse AI Tutor
=========================================================================
Checks every component of the voice stack and reports status.

Two entry points
----------------
log_health_check()          → prints results to console at startup
render_voice_health(st)     → renders a Streamlit expandable status panel
...
def _check_microphone() -> tuple[bool, str]:
    ...
    try:
        ...
        if ver >= (1, 35):
            ...
            return False, f"Streamlit {st.__version__} < 1.35 — st.audio_input() unavailable"
    except Exception as exc:
        return False, f"Streamlit check failed: {exc}"


def _check_elevenlabs() -> tuple[str, str]:
    """Returns 'ok'/'warn'/'skip' — ElevenLabs is always optional."""
    try:
    ...
    try:
        ...
        return "warn", "elevenlabs not installed (optional — pip install elevenlabs)"


def _check_groq() -> tuple[str, str]:
    """Returns 'ok'/'warn'/'skip' — Groq is always optional."""
    try:
        from backend.voice_config import cfg
    ...
    try:
        ...
        return "warn", "groq not installed (optional — pip install groq)"
...
def run_health_check() -> dict[str, Any]:
    ...
    dict with keys:
        ...
        voice_ready : bool  — True if minimum required components present
        stt_ready   : bool
        tts_ready   : bool
        errors      : list[str]
    """
...
def render_voice_health(location: Any = None) -> None:
    ...
    with target.expander(f"🎙️ Voice Status — {badge_label}", expanded=not voice_ready):
        _STATUS_STYLE = {
            "PASS": ("✅", "#2ECC71"),
            "FAIL": ("❌", "#E74C3C"),
            "WARN": ("⚠️", "#F39C12"),
