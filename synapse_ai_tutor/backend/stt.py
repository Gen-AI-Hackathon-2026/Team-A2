"""
stt.py — Speech-to-Text module for Synapse AI Tutor
======================================================
Primary  : faster-whisper  (local CPU, int8 — always available)
...
USE_GROQ_STT = True  (optional acceleration):
    audio → Groq Whisper ─── success ──→ text
                         └── failure → faster-whisper → text

Logging convention: all [STT] prefix for easy filtering.

Public API  (unchanged)
----------
    load_whisper_model(model_size)               → (model, backend_name)
    transcribe_audio(audio_bytes, language, ...)  → dict
    validate_stt_startup()                        → dict   ← NEW
"""
...
def _groq_transcribe(audio_path: str, language: Optional[str]) -> Optional[dict]:
    ...
    if not api_key:
        logger.debug("[STT] Groq key not configured — skipping Groq.")
        return None
...
def transcribe_audio(
    ...
    Pipeline (with USE_GROQ_STT=True):
        audio → Groq API → text
                └── on failure → faster-whisper → text

    Args:
        audio_bytes : Raw audio (WAV, MP3, OGG, FLAC …).
    ...
    Returns:
        dict with keys:
            ...
            ``language``    – detected / provided code
            ``confidence``  – [0, 1]
            ``error``       – None on success, string on failure
            ``provider``    – which backend produced the result
    """
    if not audio_bytes:
...
def validate_stt_startup() -> dict:
    ...
    * Model can be instantiated (without loading weights — just import check)
    ...
    * Groq API key present (if USE_GROQ_STT=True)
    ...
    if result["ready"]:
        ...
        logger.error("[STT] Startup validation FAILED — no local Whisper backend found")
