"""
tts.py — Text-to-Speech module for Synapse AI Tutor
======================================================
Primary  : gTTS  (Google TTS — free, no API key, zero rate limits)
...
Controlled centrally by backend/voice_config.py → VOICE_MODE.
...
# ---------------------------------------------------------------------------
# Provider configuration — driven by voice_config (single source of truth)
# ---------------------------------------------------------------------------
...
DEFAULT_VOICE_ID = "EXAVITQu4vr4xnSDxMaL"   # "Bella" — warm, educational tone
DEFAULT_MODEL_ID = "eleven_monolingual_v1"

# Chunk size — consistent chunking regardless of provider
...
def _strip_markdown(text: str) -> str:
    ...
    # Collapse multiple blank lines → sentence break
    text = re.sub(r"\n{2,}", ". ", text)
    # Collapse whitespace
...
def _split_into_chunks(text: str, max_chars: int = _CHUNK_MAX_CHARS) -> list[str]:
    """Split *text* into sentence-boundary-aligned chunks, each ≤ *max_chars*."""
...
def _gtts_synthesize(text: str, lang: str, output_path: Path) -> bool:
    ...
    PRIMARY provider — free, no API key, no rate limits.
...
def _elevenlabs_synthesize(text: str, voice_id: str, output_path: Path) -> bool:
    ...
    OPTIONAL FALLBACK — only called when USE_ELEVENLABS is True and gTTS fails.
    Returns True on success, False on any error.
    """
    ...
    if not api_key:
        logger.warning("[TTS] ElevenLabs API key not configured — skipping.")
        return False
...
def text_to_speech(
    ...
    lang: str = "en",
    use_elevenlabs: bool = False,   # ← default changed: False = gTTS primary
    strip_markdown: bool = True,
) -> Optional[str]:
    """
    Convert *text* to speech and return the absolute path of the generated MP3.

    Provider order: gTTS (primary) → ElevenLabs (optional fallback).
    ...
    Args:
        ...
        use_elevenlabs  : When ``True``, overrides the module-level USE_ELEVENLABS
                          ...
                          Kept for backward compatibility — callers that previously
...
def _synthesize(clean_text: str, lang: str, voice_id: str) -> Optional[str]:
    ...
    # ── Hackathon / gTTS-only mode: skip chunking entirely ────────────────────
    # gTTS handles its own internal text splitting — no API char limit.
